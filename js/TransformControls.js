	...
	var setupGizmo = function (gizmoMap) {
		...
		for (var name in gizmoMap) {
			...
			for (var i = gizmoMap[name].length; i--;) {
				...
				if (gizmoMap == gizmoTranslate && object.type === 'Line2') {
					var lineGeometryx = new THREE.LineGeometry();
					lineGeometryx.setPositions([0, 0, 0, 1, 0, 0])

					if (position) {
						lineGeometryx.translate(position[0], position[1], position[2]);
					}
					if (rotation) {
						lineGeometryx.rotateX(rotation[0]);
						lineGeometryx.rotateY(rotation[1]);
						lineGeometryx.rotateZ(rotation[2]);
					}
					if (scale) {
						lineGeometryx.scale(scale[0], scale[1], scale[2]);
					}
					object = new THREE.Line2(lineGeometryx, object.material.clone())
					object.updateMatrix();
